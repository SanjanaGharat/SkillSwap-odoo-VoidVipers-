...
class DatabaseConfig {
  ...
  async createIndexes() {
    try {
      ...
      console.log('⚠️ Continuing with server startup despite index creation issues...');
    }
  }
  ...
  async createUserSessionsIndexes(db) {
    ...
    const sessionIndexes = [
      ...
      // Temporarily skip the problematic lastActivity index
    ...
    for (const index of sessionIndexes) {
      try {
        ...
        if (error.code === 85) { // IndexOptionsConflict
          console.log(`⚠️ Skipping index creation due to conflict: ${JSON.stringify(index.key)}`);
        } else {
          throw error;
        }
      }
